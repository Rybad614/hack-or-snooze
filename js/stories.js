...
$allStoriesList.on('click', '.star', function () {
  ...
  // Checks if most recently starred story has the class of favorited
  ...
  if ($(this).hasClass('favorited')) {
    ...
    // Checks if most recently unstar story has the class of favorited
...
// remove favorites
$favoritedStories.on('click', '.star', function () {
  const article = $(this).parent()[0];
  const articleId = article.id;
  const userFavorites = currentUser.favorites;

  // loops through all stories and removes gold star and favorited story
